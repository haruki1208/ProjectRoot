...
# ジャンル名を取得
def select_genrename():
    try:
        # データベースに接続
        conn = sqlite3.connect(database_directory+dbname)
        cursor = conn.cursor()
        # ユーザーテーブルからユーザー名を取得
        cursor.execute("SELECT DISTINCT genre FROM typing_words")
        ...
        return {'error': 'Failed to fetch genre_names: ' + str(e)}
...
def get_genre_names():
    genre_names = select_genrename()   # ジャンル名を取得
    # JSON形式でクライアントにデータを返す
